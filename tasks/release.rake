...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/timeservice/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/timeservice/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
    ...
    stage('MavenCentralPublish', 'Publish artifacts to Maven Central') do
      sh 'bundle exec buildr clean mcrt:publish_if_tagged TEST=no'
    end
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/timeservice', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/timeservice').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/timeservice', milestone[:number], :state => 'closed')
        end
      end
    end
  end
